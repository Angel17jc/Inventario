...
export function useUpdateProduct() { return productMutation(async ({ id, ...data }: { id: number } & UpdateProductRequest) => { const response = await authenticatedFetch(buildUrl(api.products.update.path, { id }), { method: api.products.update.method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); if (!response.ok) await throwApiError(response, "No se pudo actualizar el producto"); return api.products.update.responses[200].parse(await response.json()); }, "Producto actualizado correctamente"); }
export function useDeleteProduct() { return productMutation(async (id: number) => { const response = await authenticatedFetch(buildUrl(api.products.delete.path, { id }), { method: api.products.delete.method }); if (!response.ok) await throwApiError(response, "No se pudo eliminar el producto"); }, "Producto eliminado correctamente"); }
