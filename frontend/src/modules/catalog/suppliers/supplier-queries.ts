...
export function useSuppliers() { return useQuery({ queryKey, queryFn: async () => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudieron cargar los proveedores");
...
function supplierMutation<T>(request: (data: T) => Promise<unknown>, successMessage: string) {
  ...
  return useMutation({ mutationFn: request, onSuccess: () => { queryClient.invalidateQueries({ queryKey }); toast({ title: "Éxito", description: successMessage }); }, onError: (error) => toast({ title: "No se pudo guardar", description: describeError(error, "No se pudo completar la operación con el proveedor."), variant: "destructive" }) });
}
...
export function useCreateSupplier() { return supplierMutation(async (data: CreateSupplierRequest) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo crear el proveedor");
...
export function useUpdateSupplier() { return supplierMutation(async ({ id, ...data }: { id: number } & UpdateSupplierRequest) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo actualizar el proveedor");
...
export function useDeleteSupplier() { return supplierMutation(async (id: number) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo eliminar el proveedor");
