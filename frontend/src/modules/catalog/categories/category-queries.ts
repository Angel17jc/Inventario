...
export function useCategories() {
  return useQuery({ queryKey, queryFn: async () => {
    ...
    if (!response.ok) await throwApiError(response, "No se pudieron cargar las categorías");
...
function categoryMutation<T>(request: (data: T) => Promise<unknown>, successMessage: string) {
  ...
  return useMutation({ mutationFn: request, onSuccess: () => { queryClient.invalidateQueries({ queryKey }); notify("Éxito", successMessage, toast); }, onError: (error) => notify("No se pudo guardar", describeError(error, "No se pudo completar la operación con la categoría."), toast) });
}
...
export function useCreateCategory() { return categoryMutation(async (data: CreateCategoryRequest) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo crear la categoría");
...
export function useUpdateCategory() { return categoryMutation(async ({ id, ...data }: { id: number } & UpdateCategoryRequest) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo actualizar la categoría");
...
export function useDeleteCategory() { return categoryMutation(async (id: number) => {
  ...
  if (!response.ok) await throwApiError(response, "No se pudo eliminar la categoría");
